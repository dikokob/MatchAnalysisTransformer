...
events_path = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.xml')) & ('f24' in x) & (sub in x)][0] #we do not need the meta
match_results_path = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.xml')) & ('srml' in x) & (sub in x)][0]

if len([path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.jsonl')) & (sub in x)]) > 0:
    metadata_path = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.json')) & (sub in x)][0]
    file_tracking_data = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.jsonl')) & (sub in x)][0]
    file_tracking_meta = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.json')) & (sub in x)][0]
    track_players_df, player_names_df = tracking_data_manipulation_new(file_tracking_data, file_tracking_meta)
elif len([path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder) if x.endswith('.dat')]) > 0:
    metadata_path = [path_folder + '\\' + x for x in os.listdir(path_folder) if ('metadata' in x.lower()) & (x.endswith('.xml')) & (sub in x)][0]
    file_tracking_data = [path_folder + '\\' + x for x in os.listdir(path_folder) if (x.endswith('.dat')) & (sub in x)][0]
    file_tracking_meta = [path_folder + '\\' + x for x in os.listdir(path_folder) if ('metadata' in x.lower()) & (x.endswith('.xml')) & (sub in x)][0]
...
event_type_id = pd.read_excel('\\\ctgshares\\Drogba\\Analysts\\Shared Assets\\f24 - ID definitions.xlsx')
qualifier_type_id = pd.read_excel('\\\ctgshares\\Drogba\\Analysts\\Shared Assets\\f24 - ID definitions.xlsx', sheet_name = 'qualifier types')
