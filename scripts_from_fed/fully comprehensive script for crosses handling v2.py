...
from tracking_data_manipulation_function import tracking_data_manipulation_new#, convert_chyronego_to_spectrum
...
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2017-18\\Premier League\\Round 8 All Data' 
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2019-20\\Premier League\\Round 2 All Data' 
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2019-20\\Premier League\\Round 1 All Data'
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2019-20\\Premier League\\Round 9 All Data' 
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2019-20\\Premier League\\Round 30 All Data'
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2019-20\\Super Cup' 
#path_folder = '\\\\ctgshares\\Drogba\\API Data Files\\2016-17\\FA Cup'
...
season = sys.argv[4]
competition = sys.argv[5]
#season = '2017-18'
#competition = 'Premier League'

if event_type == 'Cross':
    all_crosses_file_filtered = pd.concat([pd.read_excel(os.path.join('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\crosses v4'.format(season, competition), x)) for x in os.listdir('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\crosses v4'.format(season,competition)) if (sub in x) & ('~' not in x)])
    crosses_classified = pd.read_excel('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\Crosses Output.xlsx'.format(season, competition))
...
if event_type == 'Set Piece':
    all_crosses_file_filtered = pd.read_excel('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\Set Pieces with 2nd Phase Output.xlsx'.format(season,competition))
...
#all_crosses_file_filtered['home_team_name'] = list(map(lambda x: x.split(' v ')[0]), all_crosses_file_filtered['Fixture'])
#all_crosses_file_filtered['away_team_name'] = list(map(lambda x: x.split(' v ')[1]), all_crosses_file_filtered['Fixture'])
#all_crosses_file_filtered['outcome'] = np.where(all_crosses_file_filtered['Outcome']=='Successful', 1, 0)

#all_crosses_file_filtered = all_crosses_file_filtered[all_crosses_file_filtered['Cross Type']=='Open Play'].drop(['Cross Type'], axis =1).reset_index(drop=True)
#all_crosses_file['Time_in_Seconds'] = np.where(all_crosses_file['period_id']==1, all_crosses_file['Time_in_Seconds'], all_crosses_file['Time_in_Seconds'] - 45*60.0)
#all_crosses_file_filtered = all_crosses_file_filtered[(all_crosses_file_filtered['pass_situation']=='open play')].reset_index(drop = True)
#game_ids = all_crosses_file_filtered.game_id.unique().tolist()
#all_crosses_file_filtered = sys.argv[3]

#we need the crosses_file anyways to make sure that set piece crosses are treated exactly in the same way as crosses
crosses_file = pd.concat([pd.read_excel(os.path.join('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\crosses v4'.format(season,competition), x)) for x in os.listdir('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\crosses v4'.format(season,competition)) if (sub in x) & ('~' not in x)]).reset_index(drop=True)
...
events_path = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if (x.endswith('.xml')) & ('f24' in x)][0] #we do not need the meta
match_results_path = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if (x.endswith('.xml')) & ('srml' in x)][0]

if len([path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if x.endswith('.jsonl')]) > 0:
    metadata_path = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if x.endswith('.json')][0]
...
elif len([path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if x.endswith('.dat')]) > 0:
    metadata_path = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if ('metadata' in x.lower()) & (x.endswith('.xml'))][0]
    file_tracking_data = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if x.endswith('.dat')][0]
    file_tracking_meta = [path_folder + '\\' + sub + '\\' + x for x in os.listdir(path_folder + '\\' + sub) if ('metadata' in x.lower()) & (x.endswith('.xml'))][0]
    track_players_df = convert_chyronego_to_spectrum(file_tracking_data, file_tracking_meta, match_results_path)
    player_names_df = None
else:
    print('game id {} from {} {} not processed due to lack of tracking data.'.format(sub, competition, season))
    sys.exit()
...
referee_id, referee_name, venue, players_df_lineup, home_formation, away_formation, player_names_raw = match_results_file_manipulation(match_results_path)
...
#we bring in event data and join with our tracking data
#events_path = 'K:\\TK Work\\2018-19\\DVMS\\Weekly Data\\Round 21 All Data\\f24-8-2018-987792-eventdetails.xml'
#events_path = 'K:\\TK Work\\2018-19\\DVMS\\Weekly Data\\Round 19 All Data\\f24-8-2018-987778-eventdetails.xml'
event_data, game_id, game_date, away_score, away_team_id, away_team_name, home_score, home_team_id, home_team_name = opta_event_file_manipulation(events_path)
...
if metadata_path.endswith('json'):
    ...
    length = datastore['pitchLength']
    width = datastore['pitchWidth']
else:
    with open(metadata_path, encoding = 'utf-8') as fd:
        opta_meta = xmltodict.parse(fd.read()) 
    length = float(opta_meta['TracabMetaData']['match']['@fPitchXSizeMeters'])
    width = float(opta_meta['TracabMetaData']['match']['@fPitchYSizeMeters'])
#metadata = match_results_file_manipulation(metadata_path)
#metadata = pd.read_excel('C:\\Users\\fbettuzzi\\Desktop\\Chelsea\\Tracking provider files\\Bournemouth v Watford 2-1-19\\Second Spectrum\\metadata.xlsx')
...
#get a similar dataset but for events following the crosses. we probably need a function to work it out.
def get_events_after_crosses(data_event_full, data_event_crosses):
    ...
    for i in range(data_event_crosses.shape[0]):
        ...
        dat['cross_id'] = i + 1
        dat = dat[(dat.unique_event_id != data_event_crosses.unique_event_id.iloc[i]) & (~dat.type_id.isin([27,28,55,43,70])) & (dat.total_seconds_in_period >= data_event_crosses.total_seconds_in_period.iloc[i])]
    ...
    return x
#get_events_after_crosses(event_data_filtered, event_data_open_play_crosses_final)
...
#create some new columns and drop the ones we do not need anymore
...
qualifier_type_id = pd.read_excel('C:\\Users\\fbettuzzi\\Desktop\\Chelsea\\OPTA\\f24 - ID definitions.xlsx', sheet_name='qualifier types')

#here the join occurs - do we actually need to perform a join??
...
#v1, v2, v3 passed as numpy arrays
#degenerate triangles will have an area of 0
...
def largest_triangle_sampling (x_path, y_path, frame_path, temporary_x, temporary_y, temporary_frame, list_frames, current_player):
...
#    else:
    ...
    try:
        if ('Cross Type' in list(all_crosses_file_filtered)) | (event_ids[cross_id] in crosses_file.unique_event_id.tolist()):
            ...
            elif final_event_data[(final_event_data.unique_event_id==event_data_after_crosses_event.unique_event_id.iloc[0])].type_id.iloc[0] in [1,2,3,4,7,10,11,13,14,15,16,8,12,41,44,45,49,50,51,52,53,54,56,59,61,74]: 
 ...
 #    ordered_column_names = ['game_id', 'match_date', 'fixture', 'competition_name', 
 # 'Period_ID', 'OPTA_event_id', 'OPTA_qualifiers', 'team_id', 'team_name', 'opp_team_id', 'opp_team_name',
 # 'Time', 'Time_Seconds', 'Start_Window', 'End_Window', 'outcome', 'keypass', 'assist', 'player_crosser_id', 'player_crosser_name', 'player_crosser_status', 'player_crosser_position_in_pitch', 'x_crosser', 'y_crosser', 
 # 'left_or_right', 'wide_or_centred', 'before_or_after_the_box', 'inside_or_outside_the_box', 'x_Ball_final', 'y_Ball_final', 'final_ball_before_or_after_gk_area', 'final_ball_which_part_of_the_area', 'same_side_of_cross', 
 # 'Player_ID', 'Player_Name', 'Player_Team_id', 'Player_Team_Name', '@Status', 'position_in_pitch', 'Attacking_or_Defending', 'Avg_Dist_from_Centre_of_Goal', 'Start_Dist_From_Centre_of_Goal', 'Closest_Dist_From_Centre_of_Goal', 'Avg_Number_Players_within_3m', 'Avg_Dist_Closest_Player', 'Already_in_Box', 'Time_of_Entry', 'Exit_from_Box', 'Time_of_Exit', 'x_entry', 'y_entry', 'box_entry_part', 'Speed_of_Entry', 'time_spent_in_box', 'time_spent_in_box_relative_to_duration_of_window', 'time_spent_in_box_at_high_speed', 'time_spent_in_box_at_high_speed_relative_to_duration_of_window', 'time_spent_in_box_at_high_speed_relative_to_time_spent_in_box', 'window_duration', 'Number_Attacking_Players', 'Number_Defending_Players_no_GK', 
 # 'after_cross_event_type', 'after_cross_event_qualifiers', 'after_cross_players_id_involved', 'after_cross_players_name_involved',
 # 'after_cross_attacking_defending', 'after_cross_teams_id', 'after_cross_teams_name', 'after_cross_players_status', 'after_cross_players_pos', 
 # 'after_cross_players_coordinates', 'after_cross_outcome', 'after_cross_keypass', 'after_cross_assist', 
 # 'goal_event', 'goal_qualifiers', 'scorer_name', 'assister_name', 
 # 'shot_event', 'shot_qualifiers', 'shooter_name', 'passer_name', 
 # 'error_event', 'error_qualifiers', 'error_player_name',
 # 'foul_event', 'foul_qualifiers', 'foul_players_involved_name', 'foul_players_attack_defense', 'foul_outcome',
 # 'aerial_event', 'aerial_qualifiers', 'aerial_players_involved_name', 'aerial_players_attack_defense', 'aerial_outcome'] 

    #both_sources = both_sources[ordered_column_names]
    data_export.append(both_sources)

    print ('Event ID {} related to team ID {} happened in period ID {} in game ID {} completed.'.format(event_ids[cross_id], current_team_id, current_period, final_event_data.game_id.unique()[0]))

#data_export = data_export[ordered_column_names]
# reordered_column_names = ['game_id', 'match_date', 'fixture', 'competition_name', 
#  'Period_ID', 'OPTA_event_id', 'OPTA_qualifiers', 'team_id', 'team_name', 'opp_team_id', 'opp_team_name',
#  'Time', 'Time_Seconds', 'Start_Window', 'End_Window', 'outcome', 'keypass', 'assist', 'player_crosser_id', 'player_crosser_name', 'player_crosser_status', 'player_crosser_position_in_pitch', 'x_crosser', 'y_crosser', 
#  'left_or_right', 'wide_or_centred', 'before_or_after_the_box', 'inside_or_outside_the_box', 'x_Ball_final', 'y_Ball_final', 'final_ball_before_or_after_gk_area', 'final_ball_which_part_of_the_area', 'same_side_of_cross', 
#  'Player_ID', 'Player_Name', 'Player_Team_id', 'Player_Team_Name', '@Status', 'position_in_pitch', 'Attacking_or_Defending', 'Avg_Dist_from_Centre_of_Goal', 'Start_Dist_From_Centre_of_Goal', 'Closest_Dist_From_Centre_of_Goal', 'Avg_Number_Players_within_3m', 'Avg_Dist_Closest_Player', 'Already_in_Box', 'Time_of_Entry', 'Exit_from_Box', 'Time_of_Exit', 'x_entry', 'y_entry', 'box_entry_part', 'Speed_of_Entry', 'time_spent_in_box', 'time_spent_in_box_relative_to_duration_of_window', 'time_spent_in_box_at_high_speed', 'time_spent_in_box_at_high_speed_relative_to_duration_of_window', 'time_spent_in_box_at_high_speed_relative_to_time_spent_in_box', 'window_duration', 'Number_Attacking_Players', 'Number_Defending_Players_no_GK', 
...
#writer = pd.ExcelWriter('C:\\Users\\fbettuzzi\\Desktop\\Chelsea\\Tracking provider files\\Bournemouth v Watford 2-1-19\\Second Spectrum\\json\\crosses.xlsx', engine = 'xlsxwriter')
data_export = pd.concat(data_export).sort_values(['Period ID', 'Time In Seconds'])
#data_export['End Window Equals Ball End Window'] = np.where(data_export['End Window']==data_export['End Window Ball'], 1, 0)
#writer = pd.ExcelWriter('C:\\Users\\fbettuzzi\\Desktop\\Chelsea\\Tracking provider files\\Man Utd v Huddersfield 26-01-19\\Second Spectrum\\json\\crosses.xlsx', engine = 'xlsxwriter')
#writer = pd.ExcelWriter('C:\\Users\\fbettuzzi\\Desktop\\Chelsea\\crosses example.xlsx', engine = 'xlsxwriter')
#writer = pd.ExcelWriter('\\\ctgshares\\Drogba\\Advanced Data Metrics\\2019-20\\Premier League\\Set Pieces & Crosses\\tracking data crosses\\{} {}.xlsx'.format(event_data.fixture.iloc[0], 'g' + str(game_id)), engine = 'xlsxwriter')
if event_type == 'Set Piece':
    ...
    writer = pd.ExcelWriter('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\tracking data set pieces\\{} {}.xlsx'.format(season, competition, event_data.fixture.iloc[0], 'g' + str(game_id)), engine = 'xlsxwriter')
if event_type == 'Cross':
    ...
    writer = pd.ExcelWriter('\\\ctgshares\\Drogba\\Advanced Data Metrics\\{}\\{}\\Set Pieces & Crosses\\tracking data crosses\\{} {}.xlsx'.format(season, competition, event_data.fixture.iloc[0], 'g' + str(game_id)), engine = 'xlsxwriter')
data_export.to_excel(writer, index = False, sheet_name = 'Sheet1')  # send df to writer
worksheet = writer.sheets['Sheet1']  # pull worksheet object
for idx, col in enumerate(data_export):  # loop through all columns
    series = data_export[col]
    max_len = max((
...
#'max_height_ball', 'low_or_high_cross', 'avg_speed_ball_at_max_height',