...
def tracking_data_output(data, data_tracking, core_stats, season, competition):
    import numpy as np 
    ...
    if 'Cross OPTA Event ID' in list(summary_df):
        summary_df = summary_df.groupby(['Cross OPTA Event ID']).apply(numbers_in_box).reset_index(drop=True)
    else:
...
for season in seasons:
    ...
    for competition in folders_to_keep:
        ...
        data_core_stats = pd.concat([pd.read_excel(os.path.join(parent_folder, competition, 'Opta Summaries Player Level', x)) for x in os.listdir(os.path.join(parent_folder, competition, 'Opta Summaries Player Level')) if ('core stats' in x) & ('~' not in x)]).reset_index(drop=True)
